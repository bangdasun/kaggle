...
with timer('Transaction amount (monthly) features'):
    ...
    grp_3 = grp_2.groupby(['card_id']).agg(['mean', 'std'])
    grp_3.columns = ['hist_transac_a_mon_' + '_'.join(c).strip() for c in grp_3.columns.values]
    grp_3.reset_index(inplace=True)
    hist_feats = hist_feats.merge(grp_3, on='card_id', how='left')
...
with timer('Transaction (time related) features'):
    ...
    for i in range(1, 7):
        hist_feats['hist_transac_a_monthlag_last_{}_amount'.format(i)] = hist_monthsum_amount.iloc[:, -i:].sum(axis=1).values

    for i in range(1, 6):
        for j in range(i + 1, 7):
            hist_feats['hist_transac_a_monthlag_last_{}_{}_amount_ratio'.format(j, i)] = hist_feats['hist_transac_a_monthlag_last_{}_amount'.format(j)].values / hist_feats['hist_transac_a_monthlag_last_{}_amount'.format(i)].values
            hist_feats['hist_transac_a_monthlag_last_{}_{}_amount_ratio'.format(j, i)] = hist_feats['hist_transac_a_monthlag_last_{}_{}_amount_ratio'.format(j, i)].replace([np.inf, -np.inf], np.nan)
            hist_feats['hist_transac_a_monthlag_last_{}_{}_amount_log_ratio'.format(j, i)] = np.log2(hist_feats['hist_transac_a_monthlag_last_{}_{}_amount_ratio'.format(j, i)] )
...
with timer('Time decay features'):
    ...
    for m in ['sum', 'mean', 'max', 'min', 'median', 'std', 'skew']:
        hist_feats['hist_transac_a_amount_decay_{}'.format(m)] = hist.groupby(['card_id'])['transac_amount_decay'].agg([m]).values
        hist_feats['hist_transac_a_amount_month_decay_{}'.format(m)] = hist.groupby(['card_id'])['transac_amount_month_decay'].agg([m]).values
...
with timer('Transaction amount month sequence decomposition features'):
    ...
    for i in range(num):
        hist_feats['hist_transac_a_amount_month_seq_decomp_{}'.format(i)] = pca_decomp_amt[:, i]
...
with timer('Transaction amount merchant category sequence decomposition features'):
    ...
    for i in range(num):
        hist_feats['hist_transac_a_amount_merchant_seq_decomp_{}'.format(i)] = pca_decomp_amt[:, i]

with timer('Categorical mean-encoding purchase amount features'):
    for c in ['category_1', 'category_2', 'category_3']:
        hist['{}_mean'.format(c)] = hist.groupby([c])['purchase_amount'].transform('mean')
        hist_feats['hist_transac_a_{}_mean_enc_mean'.format(c)] = hist.groupby(['card_id'])['{}_mean'.format(c)].mean().values
...
with timer('Higher order transaction amount features'):
    def successive_aggregates(df, field1, field2):
        ...
        u.columns = ['hist_a_transac_' + field1 + '_' + field2 + '_' + c for c in u.columns.values]
...
with timer('Save historical transaction amount features'):
    ...
    hist_feats.to_csv('hist_transac_amount_a.csv', index=False)